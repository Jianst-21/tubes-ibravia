...
export const VillaSlider = ({ slides = [], reversed = false, blueTheme = false }) => {
  ...
  const chevronClasses = blueTheme
  ...
  return (
    <div
      className={`relative w-full transition-colors duration-500 
        ${blueTheme ? "bg-[#003B73]" : "bg-transparent dark:bg-black"}
      `}
    >
      <div
        className="
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col justify-center max-w-[512px] text-center md:text-left">
            <h2 className={`text-3xl md:text-4xl font-bold mb-[42px] ${textColor}`}>
              {title}
            </h2>

            {desc && (
              <p className={`text-[18px] leading-relaxed ${textColor} text-justify`}>
                {desc}
              </p>
            )}
          </div>
      ...
      <button
        ...
        className={`absolute top-1/2 -translate-y-1/2 left-0 z-20
          ...
          hover:scale-105 transition-all backdrop-blur ${chevronClasses}
        `}
        style={{
          marginLeft:
